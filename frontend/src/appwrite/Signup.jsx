...
const Signup = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      setMobile("");    setGender("");   setDob("");
    } catch (error) {
      if (error.response?.status === 409) {
        setAlert({ type: "error", message: "User already exists with the same details." });
      } else {
      ...
      <div className="bg-white shadow-xl rounded-xl p-10 max-w-lg w-full">
        ...
        {alert.message && (
          ...
          >
            ...
            {copyMessage && <div className="text-sm text-gray-600 mt-1">{copyMessage}</div>}
          </div>
        )}
        ...
        <form onSubmit={handleSignup}>
          <div className="mb-4">
            ...
            <input type="text" value={firstName} onChange={(e) => setFirstName(e.target.value)} className="w-full border rounded-lg px-4 py-2" required />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 font-bold mb-2">Last Name:</label>
            <input type="text" value={lastName} onChange={(e) => setLastName(e.target.value)} className="w-full border rounded-lg px-4 py-2" required />
          </div>
          <div className="mb-4">
          ...
          <div className="mb-4">
            ...
            <input type="text" value={mobile} onChange={(e) => setMobile(e.target.value)} className="w-full border rounded-lg px-4 py-2" />
          </div>
          <div className="mb-4">
            <label className="block text-gray-700 font-bold mb-2">Gender:</label>
            <select value={gender} onChange={(e) => setGender(e.target.value)} className="w-full border rounded-lg px-4 py-2" required>
          ...
          <div className="mb-4">
            ...
            <input type="date" value={dob} onChange={(e) => setDob(e.target.value)} className="w-full border rounded-lg px-4 py-2" required />
          </div>
          <button type="submit" className="w-full bg-blue-700 text-white py-2 rounded-md font-bold hover:bg-blue-800 transition">
        ...
        <div className="mt-4 text-center">
          <p className="text-gray-700">
            ...
            <button onClick={() => navigate("/login")} className="text-blue-700 font-semibold underline hover:text-blue-900 transition">
              Login here
            </button>
          </p>
        </div>
